        ...
        if (last_minute != time.getMinute()){
            ...
            for(iter it = valves->begin(); it != valves->end(); it++ ){
                ...
                if ((v.getReset() == time ) && v.getReset().getAvailable()){
                    ...
                    out << "Valve " << v.getValve().getNumber() << " " << 
                    ((v.getValve().getState()) ? "open" : "closed") << endl;

                    cout << "Valve " << v.getValve().getNumber() << " " << 
                    ((v.getValve().getState()) ? "open" : "closed") << endl;
                }
                else{
                    if (passed_check/60.0 >= v.getCheckTime()){
                        ...
                        if (v.getValve().checkState(radio)){
                            ...
                            out << "Valve " << v.getValve().getNumber() << 
                            " ok:" << 
                            ((v.getValve().getState()) ? "open" : "closed");
                            ...
                            << ". Currently " << 
                            ((v.getValve().getValveState()) ? "open" : "closed") 
                            << ". Should be " << 
                            ((v.getValve().getState()) ? "open" : "closed");
